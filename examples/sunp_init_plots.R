...
sunp_data <- arrow::s3_bucket('scores/parquet/', endpoint_override = 's3.flare-forecast.org', anonymous = TRUE) %>%
  arrow::open_dataset() %>%
...
past_obs <- sunp_data %>%
  filter(reference_datetime == '2022-11-07 00:00:00',
         depth == 1) %>%
  drop_na(observation) %>%
  select(datetime, observation)

future_predictions <- sunp_data %>%
  filter(reference_datetime == '2022-11-07 00:00:00',
         datetime >= '2022-10-17',
         depth == 1) %>%
  select(-observation)

full_data <- full_join(past_obs,future_predictions, by = 'datetime') %>%
  filter(datetime <= '2022-10-31',
         datetime >= '2022-10-10')

full_data$datetime <- as.Date(full_data$datetime)

sunp_plot <- ggplot(data = full_data, aes(datetime, observation)) +
  geom_point() +
  geom_line() +
  geom_line(aes(datetime,quantile97.5)) +
  geom_line(aes(datetime,quantile02.5)) +
  geom_ribbon(data=subset(full_data, datetime >= '2022-10-17'),
              aes(ymin=quantile02.5,ymax=quantile97.5), fill="darkslategray3", alpha=0.5) +
  geom_line(aes(datetime,median)) +
  geom_vline(xintercept = as.Date('2022-10-17'), linetype = 'dashed') +
  ylab('Water temperature (deg C)') +
  xlab('Date') +
  scale_x_date(breaks = seq.Date(from = min(as.Date(full_data$datetime)),
                                 to = max(as.Date(full_data$datetime)),
                                 by = "5 days")) +
  #annotate("text", x=as.Date('2022-10-10'), y=27, label= "Past", size = 8) +
  #annotate("text", x=as.Date('2023-01-23'), y=27, label= "Future", size = 8) +
  theme_bw() +
...
oxy_value_1m <- field_format_oxy %>%
  filter(DateTime == '2019-08-01',
         Depth == 1.5) %>%
  select(DOSat)

oxy_value_10m <- field_format_oxy %>%
  filter(DateTime == '2019-08-01',
         Depth == 10) %>%
  select(DOSat)
